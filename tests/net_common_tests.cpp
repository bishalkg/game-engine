...
bool equalSnapshots(const game_engine::NetGameObjectSnapshot& a,
  ...
  if (a.id != b.id || a.layer != b.layer || a.type != b.type) return false;
...
void testNetGameInputRoundTrip() {
  ...
  in.playerID = 123;
  in.tick = 456;
  in.move = PlayerInput::MoveLeft;
  in.fireProjectile = PlayerInput::Fire;
  in.swingWeapon = PlayerInput::Swing;
  ...
  assert(out.move == in.move);
  assert(out.fireProjectile == in.fireProjectile);
  assert(out.swingWeapon == in.swingWeapon);
}
